...
static const char *program_version = "unknown";

void command_set_program_name(const char *name)
{
	program_name = name;
}

void command_set_program_version(const char *version)
{
...
int command_execute(struct command *cmd, int argc, char *argv[])
{
	...
	// XXX: handle common options
	command_getopt(argc, argv, cmd);
